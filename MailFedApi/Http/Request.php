<?php
/**
 * This file contains the MailFedApi_Http_Request class used in the MailFedApi PHP-SDK.
 * 
...
/**
 * MailFedApi_Http_Request is the request class used to send the requests to the API endpoints.
 * 
 * @author Serban George Cristian 
 * @package MailFedApi
 * @subpackage Http
 * @since 1.0
 */
class MailFedApi_Http_Request extends MailFedApi_Base
{
    /**
     * @var MailFedApi_Http_Client the http client injected.
     */
    public $client;
    
    /**
     * @var MailFedApi_Params the request params.
     */
    public $params;
     ...
     * Constructor.
     * 
     * @param MailFedApi_Http_Client $client
     */
    public function __construct(MailFedApi_Http_Client $client)
    {
        $this->client = $client;    
    }
    ...
    public function send()
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT , 'MailFedApi Client version '. MailFedApi_Http_Client::CLIENT_VERSION);
        ...
        if ($client->isPostMethod || $client->isPutMethod || $client->isDeleteMethod) {
            ...
            $params = new MailFedApi_Params($client->paramsPost);
        ...
        $params = $this->params = new MailFedApi_Params($curlInfo);
        ...
        if ($curlCode === 0 && $client->getResponseHeaders) {
            ...
            $params->add('headers', new MailFedApi_Params($headers));
        }
        
        $decodedBody = array();
        if ($curlCode === 0 && !empty($body)) {
            $decodedBody = MailFedApi_Json::decode($body, true);
        ...
        $params->add('body', new MailFedApi_Params($decodedBody));
        
        $response = new MailFedApi_Http_Response($this);
        $body = $response->body;
    ...
    protected function sign($requestUrl)
    {
        ...
        $params = new MailFedApi_Params($specialHeaderParams);
